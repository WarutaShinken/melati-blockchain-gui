...
export default function Block() {
  ...
  const poolReward = mojo_to_melati(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_melati(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const melatiFees = blockRecord.fees
    ? mojo_to_melati(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.melatiexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.melatiexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: melatiFees ? `${melatiFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Melati blockchain
            </Trans>
          </BlockTitle>
        }
        action={
