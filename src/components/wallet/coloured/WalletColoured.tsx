...
import { AlertDialog, Card, Flex } from '@melati/core';
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_melati = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_melati = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_melati = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
    balancebox_3 +
    balance_ptotal_melati +
    balancebox_unit +
    balancebox_hline +
    balancebox_4 +
    balancebox_row +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('melati_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
