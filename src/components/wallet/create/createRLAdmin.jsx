...
import { AlertDialog } from '@melati/core';
import {
  Typography,
  Button,
  Box,
...
import { melati_to_mojo } from '../../../util/melati';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      melatiper_input.value === '' ||
      Number(melatiper_input.value) === 0 ||
      !Number(melatiper_input.value) ||
      isNaN(Number(melatiper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const melatiper = melati_to_mojo(melatiper_input.value);
    const melatiper_value = Number.parseInt(Number(melatiper));
    const userpubkey = userpubkey_input.value;
    const amount = melati_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = melati_to_mojo(fee_input.value);
