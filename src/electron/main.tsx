...
import melatiEnvironment from '../util/melatiEnvironment';
import melatiConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!melatiEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    melatiConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!melatiConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !melatiConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (melatiConfig.manageDaemonLifetime()) {
        melatiEnvironment.startMelatiDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Melati Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Melati-Network/melati-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Melati-Network/melati-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Melati-Network/melati-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Melati-Network/melati-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Melati-Network/melati-blockchain/issues',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Melati Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Melati' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Melati Blockchain' }),
            click: () => {
              openAbout();
            },
          },
