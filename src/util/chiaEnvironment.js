...
const getMelatiVersion = () => {
  let version = null;
  const exePath = getExecutablePath('melati');
  // first see if we can get a melati exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or melati is on the path
    try {
      version = child_process
